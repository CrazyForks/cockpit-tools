...
- **代理管理围绕「代理 / 账号配置」两个页签展开**：全宽可搜索列表统一管理手动代理、订阅和代理组，订阅按需展开；添加后可原地分配给指定账号或设为统一代理，已有账号独立配置仍然优先。账号配置页集中显示统一代理及账号覆盖表格，支持批量分配和恢复跟随。编辑、诊断与代理组高级参数按需打开，减少窄窗口中常驻面板对列表的挤压；连接、日志、检测与内核设置收进「诊断与设置」，账号快捷入口保留只读预览并带入当前账号进入管理。 首次使用在代理页直接展示内核下载、本地导入和添加代理入口，原地显示安装进度、取消与重试，允许先导入配置；已有配置但缺内核时提示安装，内核就绪后收成简短状态并提示下一步分配。 分配、批量绑定和恢复跟随全部成功后自动关闭弹框，单账号保存成功后同样关闭，部分失败或取消时保留结果；节点与分组下拉菜单按实际内容高度定位，下方足够时向下展开，向上展开时紧贴触发按钮。
- **Codex 账号代理活动记录**：代理预览把本地 API 请求与实时隧道日志分开展示；代理管理页按账号和通道查看路由与活跃连接，支持筛选、搜索、暂停显示、错误提示、出口检测结果及手动导出脱敏 JSON。日志由用户主动开启，仅在本次运行的内存中限量保留；不经过内置内核的直连代理无法提供连接日志。 切换账号时丢弃旧账号的活动结果；代理页面与删除影响预览中的账号身份统一遵循隐私模式。
- **新增统一代理出口**：在「账号配置」启用统一代理，或从代理列表将选中项分配为统一代理后，所有可用 OAuth 账号（含之后新增的账号）统一走同一个订阅节点或分组；配置只保存资源引用与本地快照（与代理资源共用同一套本地加密存储），账号原有绑定优先于统一代理；未独立绑定的账号继承统一代理，关闭后恢复原有出口，被引用的来源被删除时也会自动关闭并给出提示。统一代理读取失败会显示错误和重试入口；此前没有代理入口的已运行实例仍需重启才能使用新出口；API Key、模型供应商等不支持的账号不参与。 配置不完整或无法读取时保留明确错误，恢复文件后可直接重试，无需重启应用。 独立 CLI 进程沿用原网络配置；已经运行的实例专属供应商网关需要重启对应实例，才能确保采用变更后的代理。
- **Codex 账号代理采用 Mihomo 内核**：受支持的 Clash 节点及手动选择、自动测速、故障转移和负载均衡分组交由 Mihomo 原生处理，减少配置转换造成的兼容遗漏。账号代理绑定保存在本地；运行节点前需在代理页面主动安装或导入 Mihomo，不会自动安装，代理不可用时不回退直连。
- **Codex 代理资源支持订阅导入和点选绑定**：导入 HTTPS 订阅、Clash 节点配置、纯文本或 Base64 节点列表，也可手动添加 HTTP/SOCKS 代理；无需先添加账号即可管理和检测资源。来源名称自动读取订阅标题或生成，可导入后改名；账号密码代理支持常见格式、多行识别、脱敏预览、错误行定位与重复项提示。可搜索的分组与节点下拉框会回显账号已保存的绑定；选择自动分组会保留其策略；选择手动分组须明确选定成员，嵌套手动分组也逐层选择。支持“代理组 → 自动选择”的子组选择与回显，不必固定某个节点；组内阻断成员保留原语义，无法保存时在选择区显示兼容原因。也可单独绑定节点。已有绑定在明确重新绑定前保持不变。移除重复的当前选择摘要、分组路径和策略操作，分组测速放在分组下拉框右侧，节点列表内可单独测速；支持单节点和当前组内测速、进度及取消，每个检测最多10秒、最多并发3个，不提供全量测速，检测不会改变账号绑定。分组保留直接成员与子组层级，组内测速不递归展开全局自动组。订阅中声明跳过证书校验的节点可逐个明确允许；账号和资源页面提示其他代理软件的 TUN/VPN 可能接管账号出口，用户可自行决定是否继续绑定并检测。旧网卡配置不再用于节点出站。检测区分 DNS、ECH、证书、握手和连接超时。支持选择节点、组内节点、手动选择组与自动测速组，并展示内核运行时实际选中的节点。下载请求使用 Clash Meta 兼容格式，保留现代协议节点，并支持受支持的 VLESS ECH 配置；Hysteria2 节点可保留受支持的带宽与端口跳跃设置，兼容内容相同的重复端口字段及与内核行为一致的 `udp: true`；跳过证书校验仍需逐节点明确允许。手动分组中的可用成员不会被其他不支持的成员连带禁用，不可用项目会显示具体兼容原因。订阅凭据加密保存在本地，导入、刷新与检测支持取消和超时；自动刷新默认关闭，开启后每6小时检查。点击删除来源会打开独立确认弹框；刷新来源保留已有账号配置，重新绑定后才应用新配置；删除来源会同时解除关联账号的代理绑定。从资源页绑定单个账号时须先明确选择账号；批量绑定不会预选账号，确认后仅修改勾选账号。订阅还会读取响应头里的用量信息（已用、总量、到期时间），显示在来源卡片上并随每次刷新更新；订阅端未提供该信息时不显示猜测数值，暂时缺失时保留上次结果。
- **Codex 账号页新增「更多」菜单和账号级出口代理**：可拖动或点击上下箭头排序，前五项显示在顶部，其余按顺序进入「更多」，并可恢复默认布局；普通 OAuth 账号的卡片和表格在「重置」旁提供「代理」按钮，并可集中管理。Windows 和 Linux 可主动选择「带代理授权」，让内置登录窗口、令牌交换和首次配额查询走所选代理；原登录方式仍为默认，当前 macOS 构建暂不提供该入口。HTTP(S)/SOCKS5/SOCKS5h 配置用于客户端启动及 sidecar 上游请求，不对 API Key 和模型供应商账号生效；留空时保持原有代理行为，已绑定不代表已验证连通性。
- **代理出口手动检测**：保存前可测试输入的代理，查看返回 IP、请求耗时和检测时间。检测访问 api64.ipify.org，不携带账号 Token，有超时限制，代理失败时不会改为直连重试；结果只代表本次检测请求。
- **一个节点可批量绑定到多个 Codex 账号**：在「账号配置」勾选账号，或直接全选当前筛选结果，即可把同一节点或分组一次性绑定给这些账号；代理列表也可从节点直接分配给明确勾选的账号。弹框内保留草稿出口检测，跳过已经使用该节点的账号，确认后默认覆盖其他现有绑定，并在执行前明确提示，并在部分失败时逐条给出账号与原因、自动定位错误提示。取消会停止后续账号，正在写入的账号完成后仍同步实际结果，随后可关闭或重试；出口检测期间锁定来源和提交，避免检测结果与所选节点混淆。每个账号仍保留各自独立的绑定与本地隧道；已有稳定本地入口的客户端在新连接中使用变更后的配置，启动时未接入该入口的客户端首次绑定后需重启一次。
- **代理页补充批量解绑、统一代理影响预览与连接流量**：批量选择除批量绑定外也可一次性解除所选账号的代理绑定，确认弹框显示将解除的账号数量，执行时逐条返回结果并列出部分失败的账号与原因，解绑后按统一代理或原有默认出口生效；开启统一代理前先读取真实影响范围，弹框显示将改用统一代理的账号数量与继续保留独立代理的账号数量，读取失败可在弹框内重试；活动连接列表显示每个连接的上传与下载量。
- **代理页面可按需安装内核**：用户主动下载支持的 Mihomo 1.19.31 官方安装包，或导入对应本地压缩包，支持校验、进度、最终启用前取消与重试。首次启动检查单独展示进度并允许取消，启动超时、无法运行和版本不匹配会分别提示。没有账号也能查看支持版本、安装包大小和已安装版本；离开页面后后台继续安装，重进页面可恢复查看当前状态。内核保存在应用用户目录，不修改系统代理、不发送账号 Token；主安装包不内置内核，也不自动检查更新或静默安装。
- **账号快捷入口与运行记录**：账号卡片或表格点击「代理」打开只读预览，再从预览进入「账号配置」并定位该账号；按钮统一显示「代理」，沿用现有样式区分绑定状态，悬浮提示保留完整出口信息，代理账号列表显示原始订阅标识与已保存节点，订阅徽章沿用普通账号列表样式，不显示额度。运行记录与最近 8 条已完成的本地 API 请求在代理页面内按账号展示，包含通道、端口、模型、结果、HTTP 状态和耗时；账号与 API 服务通道内容一致时合并为一条状态行，通道不同时拆开展示。请求记录不证明实际出口 IP，出口检测仍单独展示。
- **重新授权已有账号默认沿用账号出口**：普通 OAuth 账号点「重新授权」时，内置登录窗口、令牌交换与首次配额查询默认使用该账号当前生效出口（账号独立绑定优先，其次统一代理），弹框内显示本次使用的出口并允许关闭或改用其他地址；本次登录填写的地址只作用于这次授权，不会静默改写账号已有绑定。此前「重新授权 + 代理」会在令牌交换阶段直接失败，现已修复。macOS 当前构建仍不提供内置授权窗口代理，界面按实际能力说明。 仅末尾斜杠不同的等价代理地址可复用同一进行中的授权会话。 更改授权代理时会先完成旧会话取消，再准备新的授权链接。
- **代理来源可设置默认项**：在代理资源页把某个分组或节点设为该来源的默认项，账号与统一代理选择这个来源时会自动把它带入草稿（点击「绑定账号」前不写入）；来源刷新后默认项失效会被清除并提示重新设置。
- **代理资源页支持自建多节点策略来源**：可从多个已导入来源勾选受支持的节点组合成自己的策略，选择 `select`（手动选择）、`fallback`（故障转移）、`url-test`（自动测速）或 `load-balance`（负载均衡），最多 64 个成员，成员顺序按添加顺序保留，可用上移、下移调整。保存后可再次编辑，编辑时会回填已保存的检测地址、间隔、超时、容差与懒检测，并且只提供该策略类型真正生效的参数（手动选择不做健康检查，负载均衡不支持超时），留空继续使用内核默认值。策略来源与订阅、手写来源并列，可改名、设置默认项，并被账号或统一代理像其他来源一样引用；成员按真实来源身份恢复，原来源被删除后策略继续使用已保存的节点副本，再次编辑保存也不会丢掉这些成员，成员同名时会在弹框里被拦截；已保存成员在当前来源里找不到可用节点时，卡片会显示失效数量并在下次保存时移除，删除策略前同样列出会解除绑定的账号与统一代理影响。 新建与编辑代理组支持独立来源筛选，按节点名称、服务器域名/IP 或端口搜索，不再因来源名称命中而列出全部节点；候选项显示服务器地址，疑似订阅公告仅作提醒，不自动移除。 搜索支持用空格分隔多个关键词，所有关键词均需命中，顺序不限（例如「美国 家宽」）。
- **账号出口状态区分生效方式**：代理页的账号列表与详情按「独立代理 / 统一代理 / 默认出口 / 已失效」显示实际生效方式，已保存的绑定在来源或节点失效时会显示为已失效，不再用绿色「已绑定」等同可用。
- **受管客户端使用稳定本地入口，换代理不再重启**：已有生效出口（账号独立绑定或统一代理）的账号在客户端启动时注入固定本地入口，端口记录在应用数据目录中持久化、宿主重启后优先复用，被占用时按确定性顺序改用下一个端口。换节点、换策略、解绑和统一代理变化只影响新连接，已建立的连接继续走旧出口直到结束。未绑定账号保持原有启动出口，应用全局代理、自定义 `--proxy-server`/PAC 参数与系统代理都不被改写，这类客户端首次绑定后需要重启一次；入口创建失败时不注入代理参数，客户端照常启动。 解除最后一个生效代理后，再次启动客户端会恢复原有代理环境变量与 PAC 设置。
- **删除代理来源前展示影响范围**：删除订阅或手动来源时先列出会解除绑定的账号数量与名称、统一代理是否会随之关闭，以及哪些自建策略引用了该来源（这些策略继续使用已保存的节点副本，再编辑保存时也保留），并说明删除后按「账号绑定 > 统一代理 > 原有默认出口」重新解析出口；影响范围读取失败时可在弹框内重试，读取成功前不能确认删除。 来源并发变更时须重新读取影响；sidecar 同步失败会保留来源供重试，不会误报删除成功。 删除或部分失败后会回读账号绑定；回读失败可重试，不会重复删除已移除的来源。
- **代理凭据不进入账号响应、导出、浏览器缓存、启动注入日志和 sidecar 备份**：已保存配置沿用应用的加密账号存储，界面仅接收协议/服务器/端口摘要；sidecar 认证文件使用私有原子替换，不再生成明文 `.bak`，Unix 文件创建权限为 `0600`。已保存代理按账号 ID 检测；读取旧缓存时清除原始代理字段，导入账号导出文件后需重新配置代理凭据。
- **账号代理检测可取消，并展示本地运行状态**：离开页面或点「取消检测」会停止进行中的出口检测；页面会显示账号隧道和桌面转接处于空闲、启动中、运行、已停止或内核未装，启动超时后也会清除启动中状态。桌面固定入口与按需启动的代理内核分别显示：入口已监听但尚无请求时显示就绪与等待请求，并展示实际入口端口、有效连接请求数及最近转接结果；失败、停止与缺少内核不会被就绪状态掩盖，已停止的入口会在下次启动客户端时重新建立。代理预览按实际出口区分账号独立代理、继承统一代理和未配置，切换账号不会沿用上一个账号的状态；这些本地状态不代表外网连通或官方客户端请求已成功。常见的 VLESS、VMess、Trojan、无插件 Shadowsocks SIP002、Hysteria2 和 TUIC 分享链接解析通过后即可保存；未支持的协议参数会明确拒绝。真正运行需要 Mihomo 1.19.31，可在代理页面主动下载或本地导入，主安装包不内置内核。已有账号刷新 Token 会使用绑定代理；仅凭 refresh token 导入仍沿用原有网络路径。 检测中切换账号会取消旧检测，新账号可立即继续操作。

### 变更

- **Codex 上下文与压缩统一为 90% 口径**：受管模型目录、Provider 与 API 服务目录、逐模型设置以及 config.toml 顶层配置现在都会同时给出自动压缩阈值（取上下文窗口的 90%），不再出现「只有上下文、没有压缩阈值」或「压缩阈值等于/超过上下文」的配置；`516K` 预设的压缩阈值由 `460000` 调整为 `464400`，`1M` 预设保持 `1000000 / 900000`。自定义上下文只填写窗口时会自动补全 90% 的压缩阈值，「跟随官方」仍然不写入任何覆盖值。

- **新建实例默认选择空白实例**：新建实例弹框默认选中“空白实例（不复制）”，仍可手动选择复制来源实例；主动使用“复制实例”入口时仍保持复制模式。

### 修复

- **修复 WorkBuddy 加密登录文件导致切号和本机导入失败**：通过已安装官方客户端的本地密钥接口读写加密凭据字段，保留其他账号和加密格式；不支持或损坏的密文仍会停止覆盖并保留登录文件，密钥准备在后台执行并限制超时。

- **Codex 实例操作列不再遮挡相邻列**：操作按钮随表格横向滚动，不再在窄窗口下固定覆盖套餐、速度和 PID 列；其他平台保持原有行为。

- **账号池失败不再误显示为无关供应商异常**：缺少逐账号诊断时，弹框按请求模型与 API Key 展示池级问题，不再把 DeepSeek 等所有成员都标成不可用。模型不支持及策略排除不再提供恢复操作；全部恢复只处理当前可见且可恢复的账号故障，凭据失效仍提供重新授权。

- **账号异常弹框的「恢复」现在会立即清掉异常行**：恢复会先重置宿主侧的调度、冷却与账号池诊断状态并马上返回，凭据刷新和网关重载在后台继续执行，失败时在同一个弹框内提示并可重试；被恢复的账号会在短时间内不再出现在账号池诊断里，恢复过程中仍在执行的旧请求产生的失败事件不会再让异常行立刻回来。

- **启动预览弹框不再卡在加载中**：从预览弹框启动绑定实例时会先关闭预览再执行启动事务，主按钮不会再停在“加载中”，启动期间「关闭」也始终可用，可以随时退回启动进度弹框。

- **启动过程中打开右上角快捷设置会显示加载状态，而不是空白弹层。**

- **macOS 关闭上一个客户端不再无限等待**：优雅退出步骤增加超时并回落到强制关闭，自动化权限弹窗或 System Events 无响应不会再卡住实例启动。

- **修复 Codex 未知模型被按 1M 上下文上报**：没有官方目录条目的模型（自定义模型、第三方供应商模型等）此前会带上 100 万 Token 的上下文上限，现在兜底与默认上下文一致（272K 档），1M 级窗口只能来自模型目录或你的显式设置。

- **修正 GPT-6 Astra / Sol / Luna 的上下文窗口**：由 105 万 Token 修正为 256K，并配套 230400 的自动压缩阈值；Ollama 兼容端点与内置模型注册表同步。

- **修复 apikey.fun Codex 配置模板把压缩阈值写成等于上下文窗口**：阈值等于窗口意味着永不触发自动压缩，现在按上下文的 90% 生成。

- **修复账号代理隧道重启后 API 服务请求失败**：sidecar 的账号认证文件里记录了本地代理端口，而网关只在 config/manifest 指纹变化时重启。隧道重建会换到新的随机端口，正在运行的 sidecar 会继续拨已经消失的旧端口，导致之后的请求一直返回 502 `connection refused`，必须手动重启 API 服务才能恢复。现在账号本地代理端口会参与重启指纹，账号隧道重建后会后台重新核对网关，本地回环代理被拒也会触发 sidecar 自动重启。

- **Codex API 服务估算采用最新 GPT-5.6 Sol 价格**：标准档每百万 Token 的输入／缓存输入／输出价格调整为 $4／$0.40／$20，`gpt-5.6` 别名同步生效；快速档与长上下文估算按对应官方价格计算。有完整 Token 明细时，GPT-5.6 与 GPT-6 的缓存写入按官方输入价的 1.25 倍估算。已保存的 Sol 旧默认价会更新，手动价格保留，历史估算在后台重算。

- **鹈鹕测智和唤醒独立于 API 服务运行**：鹈鹕测智恢复由所选账号直连官方上游；两项功能均独立准备账号，不读取 API 服务的账号缓存、拒绝状态或网络设置，改用账号绑定代理或全局／系统代理，不再受服务调度、冷却状态及 sidecar 运行状态限制。

- **防止滚动弹框中的控件聚焦引发位置跳动**：将 Codex 供应商模型同步开关，以及同类导入、敏感信息导出和唤醒账号选择控件的焦点位置固定在各自可见行内。

- **修复供应商网关流式回答重复显示**：同一条消息从增量输出到结束始终保持相同 ID，避免客户端将结束时的完整内容识别为第二条回答，同时保留第三方 ID 格式兼容处理。

- **修复同邮箱的 Codex OAuth 与 Grok 供应商账号相互覆盖**：OAuth 授权和 Token 导入不再复用供应商账号；历史混合记录重新导入凭据后会保存为独立 OAuth 账号，原记录及其引用保留。

- **修复 API 服务「恢复」后账号立刻又变成不可用**：恢复现在与「取消勾选再重新加入集合」走同一套成员保存路径，本地网关会按新成员重载该账号，而不再只清掉仍在内存里的调度标记。

- **修复切回官方 Codex 账号后第三方供应商配置残留**：把第三方或自建供应商账号写入 Codex profile 时，`model_provider` 及其 `[model_providers.<id>]` 条目会留在 `config.toml` 里，而官方账号投影会把未知 provider id 当作你自己的配置保留，于是账号已经切回官方，客户端仍按上一个供应商发请求，界面也继续显示该供应商。现在写入前按 profile 记录 `model`、`model_provider` 与 provider 条目原值，切回官方账号时按同一份记录还原；记录按 profile 隔离，不依赖具体供应商标识。 连续切换供应商不会恢复已清理的受管供应商引用，用户后来手动更改的供应商与模型选择保持不变。
