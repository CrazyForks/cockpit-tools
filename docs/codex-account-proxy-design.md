...
节点链接解析通过后即可保存；真正运行和出口检测仍需独立进程内核。节点解析器已经注册，临时节点检测接入独立进程运行层：版本固定检查、随机本地认证、回环 mixed 入站、stdin 配置、8 秒启动超时（另有 3 秒版本检查）、句柄释放时 kill_on_drop、显式停止等待回收、Windows CREATE_NO_WINDOW。运行时不搜索 PATH、不隐式下载安装。内核缺失会明确报错。新增用户主动安装到应用用户目录的路径；兼容既有可执行文件旁、`proxy-engine/` 以及 macOS 安装包 `Contents/Resources/proxy-engine/` 查找。主安装包不内置内核。
...
- 「安装内核」从官方 GitHub release 下载；「本地导入」选择对应官方 ZIP/gz，文件不会上传。两种入口采用同一套归档校验和安装流程，安装位置为应用用户目录，不修改系统代理、不发送账号 Token。
...
- 选择区采用「代理组 → 节点」下拉，可搜索、显示测速结果、仅看可选项、按延迟排序。分组内仅显示直接节点与子组；选子组后再显示该层成员；不支持的分组策略仍可浏览其独立可用节点。未选分组时可直接选节点，但不提供全量测速。需要绑定分组策略时必须主动点击相应操作，再完成所需手动子组选择。
- 当前选择是草稿，点击绑定才写入账号；来源改名、测速和筛选不会重新绑定账号，已绑定配置继续保持快照行为。

## 单节点和组内延迟检测

- 仅用户触发：单节点或当前选定分组的直接可用节点，去重后最多并发3个；无全局测速入口，不自动修改节点选择或路由。
- 使用独立临时内核及本地代理连接访问 `https://www.gstatic.com/generate_204`，禁止直连回退和重定向，不发送账号 Token、Cookie。只有 HTTP 204 算成功。数字是本次 HTTP 请求耗时（包含连接建立，不含内核启动），不是下载带宽、ICMP 延迟或账号业务可达性保证。
- 每节点整个后端检测（包含读取和临时内核准备）最多10秒。取消停止排队及进行中的请求；关闭页面/更换来源取消旧批次，迟到结果丢弃，临时内核在取消和超时后释放。
- 结果仅存在页面内存，关联来源 ID、配置 revision 和节点 ID；更新/改名等 revision 变化时清除旧结果，避免把旧配置结果误显示为新配置状态。出口 IP 检测仍使用单独的 ipify 流程。
- 参考：Clash Verge Rev 的选择界面与 `src-tauri/src/config/prfitem.rs` 订阅文件名处理；来源已在中英文 README 致谢。


## 节点证书选项与来源网络兼容

- Clash 订阅的 `skip-cert-verify: true` 保留为待批准节点，初次导入不允许绑定/检测。用户选中节点后明确开启才可使用；绑定快照记录获批节点，不能通过向快照注入跳过证书验证参数来绕过声明。禁用后新绑定/检测被阻止，已有绑定仍保持原快照，应重新绑定才改变已有账号。
...
账号卡片和表格的代理入口打开紧凑预览。预览中显示当前绑定、出口 IP、运行记录和近期请求；来源、分组、节点为草稿选择，可单独检测，点击绑定后才写入账号。代理管理页账号列表复用普通账号订阅徽章的样式，但显示后端原始订阅标识，不显示额度；同时展示绑定状态与脱敏节点摘要。
