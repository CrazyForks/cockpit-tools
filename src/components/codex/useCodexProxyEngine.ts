...
export function useCodexProxyEngine() {
  ...
  const run = async (kind: 'install' | 'import' | 'cancel') => {
    ...
    try {
      if (kind === 'cancel') {
        ...
        if (mounted.current) setStatus((previous) => ({ ...next, installedVersion: next.installedVersion ?? previous?.installedVersion ?? null }));
      }
    } catch (caught) {
  ...
  const readiness = proxyEngineReadiness(status, errorKey === 'codex.proxy.engine.statusUnavailable');
