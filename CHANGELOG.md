...
- **Proxy management centers on Proxies and Account setup**: a full-width searchable list brings manual proxies, subscriptions and proxy groups together, with subscriptions expanded on demand. Add a proxy, then assign it in place to selected accounts or use it as the shared default; individual account settings remain authoritative. Account setup shows the shared proxy and a table of account overrides, with batch assignment and restoration of shared settings. Editing, diagnostics and advanced group options open only when needed, leaving more room for lists in small windows. Connections, logs, tests and engine settings remain available under Tools & settings; account shortcuts retain the read-only preview and carry their account context into management. First-use guidance offers engine download or local import alongside Add proxy, with progress, cancellation and retry in place; configuration can be imported before installation. Existing configurations show an install prompt when the engine is missing, and a ready engine collapses to a compact status with the next assignment step. Assignment, batch binding and restore-follow dialogs close automatically after complete success, as does a successful single-account save; partial failure or cancellation retains results. Node and group menus use their content height to prefer opening below when space allows and remain attached to their trigger when opening above.
- **Per-account Codex proxy activity**: the proxy preview separates local API requests from live tunnel logs; the proxy page shows recent routes and active connections by account and channel, with filtering, search, pause, error hints, exit-check context and a manual sanitized JSON export. Logging is opt-in, kept in a bounded in-memory session, and unavailable for direct proxies that bypass the built-in engine. Switching accounts discards stale activity results; account identities throughout proxy pages and deletion previews follow privacy mode.
- **One shared proxy egress for all Codex accounts**: enable it in Account setup or assign a resource as the shared proxy to route every eligible OAuth account through one subscription node or group, including accounts added later. Only a resource reference and a local snapshot are stored, encrypted with the same local store as proxy resources, so per-account bindings take precedence over the shared exit; accounts without their own binding inherit it, and turning it off restores their previous exit, and deleting the referenced source also turns it off with a notice. A shared-exit read failure shows an error and retry action; running clients without a proxy entry from launch still need a restart to use the new exit, and API-key, provider and other unsupported accounts are never included. Incomplete or unreadable settings remain an explicit, retryable error; recovery does not require restarting the app. Standalone CLI processes retain their existing network settings; an already-running per-instance provider gateway needs a restart of that instance to adopt a changed proxy.
- **Codex account proxies use Mihomo**: run supported Clash nodes and manual, latency, failover or load-balancing groups with the native Mihomo engine, reducing configuration translation losses. Account proxy bindings are saved locally; install or import Mihomo explicitly from the proxy page before running nodes. Installation is never automatic, and unavailable proxies do not fall back to a direct connection.
- **Codex proxy resources support subscription import and click-to-bind selection**: Import HTTPS subscriptions, Clash node configurations, plain-text or Base64 node lists, or manually add HTTP/SOCKS proxies; resources can be managed and tested before adding an account. Source names are read from subscription metadata or generated automatically and can be renamed later. Credential proxies support common formats, multiline recognition, masked previews, error-line navigation and duplicate notices. Searchable group and node dropdowns restore the account’s saved binding. Automatic groups retain their routing policy; manual groups require an explicit member, including at each nested manual group. A parent group can select and restore an automatic subgroup without pinning a node. Blocking members retain their original behavior, and incompatible selections explain why saving is unavailable beside the picker. A node can also be bound independently. Existing bindings stay unchanged until explicitly rebound. Group checks sit beside the group dropdown, and individual nodes can be checked from the node list, with progress and cancellation. Test one node or the current group with progress and cancellation, a ten-second deadline per check and up to three concurrent checks; there is no all-source test, and checks never change account bindings. Groups retain their direct members and subgroup hierarchy; group checks do not recursively expand global automatic groups. Nodes requesting unverified TLS can be explicitly allowed individually. Account and resource pages warn that another proxy app’s TUN/VPN may capture account traffic; users can still bind and check the exit IP. Legacy interface settings no longer bind node egress. Checks distinguish DNS, ECH, certificate, handshake and connection timeout failures. Select a node, a group member, a manual selection group, or an automatic latency group, with the engine's actual selected node shown while running. Downloads request Clash Meta-compatible subscription output to retain modern protocol nodes, with supported VLESS ECH settings preserved; Hysteria2 nodes preserve supported bandwidth and port-hopping settings, including matching duplicate port fields and `udp: true` when it matches the engine behavior; certificate verification bypass still requires explicit per-node approval. Usable members of manual groups remain selectable when other members are unsupported, and unavailable items explain their compatibility limits. Subscription credentials are encrypted locally; imports, refreshes, and probes support cancellation and timeouts. Automatic refresh is off by default and checks every six hours when enabled. Deleting a source opens a separate confirmation dialog. Refreshing a source preserves existing account configurations; rebinding explicitly applies updated configurations. Deleting a source unbinds its proxy from linked accounts. Binding from a resource first requires an explicit account choice; batch binding preselects no accounts and changes only those checked before confirmation. Subscriptions also read the usage headers (used, total, expiry) and show them on the source card, refreshed on every update; no numbers are invented when a provider omits them, and a temporarily missing header keeps the previous reading.
- **Codex accounts now have a More menu and per-account egress proxies**: drag tabs or use the up/down arrows to reorder them, with the first five shown in the top row and the rest listed in More in order; restore the default layout at any time. Ordinary OAuth accounts have a Proxy button beside Reset in cards and tables, plus a shared management entry. On Windows and Linux, users can opt into proxy authorization in the built-in sign-in window; token exchange and the first quota check then use the selected proxy. The existing sign-in path remains the default, and this option is unavailable in the current macOS build. HTTP(S)/SOCKS5/SOCKS5h settings apply to client launches and sidecar upstream requests; API-key and provider accounts are excluded, and leaving the setting empty preserves the existing proxy behavior. A bound proxy is not a connectivity guarantee.
- **Manual proxy egress checks**: test an entered proxy before saving and view the returned IP, request latency, and check time. Checks contact api64.ipify.org without account tokens, have a timeout, and never retry directly when the selected proxy fails. Results describe the test request only.
- **Batch-bind one node to several Codex accounts**: tick accounts in Account setup, or select all accounts in the current filter, then bind one imported node or group to every chosen account at once. The Proxies page also lets you assign a node directly to explicitly selected accounts. The dialog keeps the draft egress check, leaves accounts already on that node untouched, overwrites other existing bindings on confirmation with a clear warning, and reports per-account results with an automatically revealed error message when part of the batch fails. Cancellation stops subsequent accounts while preserving the outcome of any write already in progress, then allows closing or retrying. Source changes and submission stay disabled during egress checks so results match the selected node. Each account keeps its own binding and local tunnel. Clients already using the stable local entry apply changes to new connections; clients launched without that entry need one restart after their first binding.
- **The proxy page adds batch unbinding, shared-exit impact previews and connection traffic**: batch selection can unbind the chosen accounts as well as bind them; a confirmation names how many accounts lose their binding, results are reported account by account with the failing accounts and reasons, and each account then resolves its exit through the shared proxy or its previous default. Enabling the shared exit reads its real impact first, and the dialog names how many accounts will switch to it and how many keep their own binding, with a retry inside the dialog when that read fails. The active-connections list also shows each connection's upload and download amounts.
- **Install the proxy engine from its management page**: explicitly download the supported Mihomo 1.19.31 official package or import the matching local archive, with verification, progress, cancellation before final activation, and retry. First-launch checks have their own progress state and can be cancelled; startup timeouts, execution failures and version mismatches are reported separately. The page shows the supported version, package size and installed version even without an account. Installation continues when leaving the page; returning restores its current status. Packages are stored in the application user directory without changing the system proxy or sending account tokens. The main installer does not bundle the engine, and no background update check or automatic installation is enabled.
- **Account shortcut and run history**: clicking Proxy on an account card or table row opens a read-only preview; its management action opens Account setup on that account; the control keeps the Proxy label, with binding state shown by its existing style and the full exit details in its tooltip, and the account list keeps the raw subscription marker with the saved node and no quota rows. Run history and the account's eight most recent completed local API requests live on the proxy page and show channel, port, model, outcome, HTTP status and duration; identical account and API service channels collapse into one status row and stay separate when they differ. Request records do not prove actual egress IP; the explicit egress check remains separate.
- **Re-authorizing an account now reuses that account's exit by default**: for an ordinary OAuth account, Re-authorize uses the account's current effective exit (its own binding first, then the unified proxy) for the built-in sign-in window, token exchange and first quota query. The dialog shows which exit this sign-in uses and lets you turn it off or switch to another address; an address entered here applies to this sign-in only and never silently rewrites the saved binding. Re-authorizing with a proxy previously failed at the token exchange, which is now fixed. The current macOS build still has no built-in sign-in proxy and says so. Equivalent proxy addresses with or without a trailing slash reuse the same in-progress sign-in session. Changing the login proxy waits for the previous session to be cancelled before preparing another authorization link.
- **A proxy source can carry a default selection**: set a group or node as that source's default on the resources page; when an account or the unified proxy picks that source, the default is filled into the draft (nothing is written before Bind account). If the default stops resolving after a source refresh it is cleared with a prompt to choose again.
- **Self-built multi-node strategy sources on the proxy resources page**: pick supported nodes from several imported sources into one strategy (`select`, `fallback`, `url-test` or `load-balance`) with up to 64 members, kept in the order you add them and movable up or down. A saved strategy can be edited again; the dialog restores its saved health-check address, interval, timeout, tolerance and lazy setting and only offers the fields that type actually applies (manual selection runs no health check, load balancing has no timeout), while blank fields keep the engine defaults. A strategy sits alongside subscriptions and manual sources: it can be renamed, carry a default selection and be referenced by an account or the shared exit like any other source. Members are restored by their real source identity, so deleting the original source keeps the strategy's saved node copies and a later edit-save no longer drops them, while members that share a name are blocked in the dialog. Members that no longer match a usable node are counted on the card and dropped on the next save, and deleting a strategy previews the accounts that lose their binding and whether the shared exit is switched off. Group creation and editing separate source filtering from node-name, server-host/IP and port search, so matching a source name no longer includes every node. Candidates show their server endpoint; possible subscription notices are advisory and are not automatically removed. Search accepts whitespace-separated keywords and requires all of them to match, in any order.
- **Account exit states name the effective route**: the proxy page's account list and detail view now show independent proxy / unified proxy / default exit / unavailable, so a saved binding whose source or node is gone reads as unavailable instead of looking like a healthy green "bound".
- **Managed clients use a stable local entry, so switching proxies no longer needs a restart**: an account with an effective exit (its own binding or the unified proxy) gets a fixed local entry when the client starts. Port records are persisted in the app data directory, reused after a host restart and advanced in a deterministic order when occupied. Changing nodes, strategies, unbinding and unified-proxy changes only affect new connections, while established connections keep their previous exit until they finish. Accounts without a binding keep their original launch egress: app-wide proxy settings, custom `--proxy-server`/PAC arguments and the system proxy are never rewritten, and those clients need one restart after their first binding. If the entry cannot be created, no proxy argument is injected and the client still starts. After removing the last effective proxy, a new client launch restores the original proxy environment and PAC settings.
- **Deleting a proxy source now previews its impact**: before a subscription or manual source is deleted, the dialog lists how many accounts will lose their binding and their names, whether the unified proxy will be switched off, and which self-built strategies reference that source (they keep working from their saved node copies, including when edited and saved again). It also explains that the egress is then resolved as account binding > unified proxy > previous default exit. A failed read can be retried inside the dialog, and deletion stays disabled until the impact is known. Concurrent source changes require the impact to be read again; sidecar synchronization failures keep the source available for retry rather than reporting a successful deletion. Account rows are reread after deletion or partial failure, and a failed reread can be retried without deleting the source twice.
- **Proxy credentials stay out of account responses, exports, browser caches, launch-injection logs and sidecar backups**: saved proxy settings continue to use the application's encrypted account storage, while the UI receives only protocol/server/port metadata. Sidecar authentication files use private atomic replacement without plaintext `.bak` files; Unix files are created as mode `0600`. Testing a saved binding uses its account ID. Previously cached proxy fields are removed when read; proxy credentials must be reconfigured after importing an account export.
- **Codex account proxy checks can be cancelled and show live local runtime**: leaving the page or using Cancel check stops an in-flight egress test; the page shows whether the account tunnel or desktop bridge is idle, starting, running, stopped, or missing the local engine, and a timed-out start clears its starting status. The stable desktop entry and its on-demand proxy engine are shown separately: a listening entry with no requests is ready and waiting, with its actual port, valid connection-request count and latest forwarding result. Readiness does not hide failures, stopped listeners or a missing engine, and a stopped entry is recreated on the next client launch. The preview distinguishes an account override, an inherited unified proxy and no proxy from the effective route, and switching accounts never reuses the previous account’s status. These local observations do not establish internet connectivity or successful official-client requests. Common VLESS, VMess, Trojan, plugin-free Shadowsocks SIP002, Hysteria2 and TUIC share links can be saved after local parsing; unsupported protocol options are rejected. Running nodes requires Mihomo 1.19.31, available through an explicit download or local import on the proxy page; it is not bundled in the main installer. Existing-account token refresh uses its bound proxy; refresh-token-only import keeps its previous network path. Switching accounts cancels the previous check and keeps the newly selected account usable.

### Changed

- **Codex context and compaction now follow one 90% rule**: the managed model catalog, provider and API Service catalogs, per-model settings and the top-level config.toml entries now always carry an auto-compact threshold equal to 90% of the context window, so "context without a threshold" and thresholds equal to or above the window are gone. The `516K` preset threshold moves from `460000` to `464400`, and `1M` stays `1000000 / 900000`. Custom entries that only fill in a window now derive the 90% threshold, and "Follow official" still writes no override at all.

- **New instances default to blank initialization**: the new-instance dialog now selects “Blank instance (no copy)” by default. Copying an existing instance remains available, and the dedicated duplicate action still selects copy mode.
...
- **Manual recovery in the account status dialog now clears the abnormal row immediately**: recovery resets the host-side scheduling, cooldown and account-pool diagnostic state first and returns right away, while the credential refresh and gateway reload continue in the background and report a failure you can retry from the same dialog. Recovered accounts also stay out of the account-pool diagnostics for a short window, so a failure event raised by an already running request can no longer make the row reappear the moment it is cleared.

- **The launch preview no longer sticks on its loading state**: starting a bound instance closes the preview before the launch transaction runs, the primary button always leaves its loading state, and 关闭 stays available so a long launch can still be dismissed into the progress dialog.

- **Opening the top-right quick settings while a launch is in progress shows a loading state instead of an empty panel.**

- **macOS no longer waits indefinitely when closing the previous client**: the graceful quit step now times out and falls back to force close, so an automation permission prompt or an unresponsive System Events cannot block relaunching an instance.

- **Fixed Codex reporting unknown models with a 1M context**: models without an official catalog entry (custom models, third-party provider models) previously carried a one-million-token context ceiling. The fallback now matches the default context window (the 272K tier), so 1M-class windows can only come from the model catalog or from your explicit setting.

- **Corrected the GPT-6 Astra / Sol / Luna context window**: it is 256K instead of 1.05M, paired with a 230400 auto-compact threshold; the Ollama-compatible endpoint and the built-in model registry match.

- **Fixed the apikey.fun Codex config template writing a compact threshold equal to the context window**: a threshold equal to the window means auto-compaction never triggers; it is now generated at 90% of the window.

- **Fixed API Service requests failing after an account proxy tunnel restarts**: the sidecar keeps each account's local proxy port in its auth file, while the gateway previously restarted only when the config/manifest fingerprint changed. A rebuilt tunnel uses a new random port, so the running sidecar kept dialling the old one and every later request failed with 502 `connection refused` until the API Service was restarted by hand. The account's local proxy port now participates in the restart fingerprint, a rebuilt account tunnel triggers a background gateway recheck, and a refused loopback proxy dial also triggers the automatic sidecar restart.

- **Codex API Service estimates use current GPT-5.6 Sol prices**: Standard input / cached input / output rates are now $4 / $0.40 / $20 per million tokens, including the `gpt-5.6` alias; Fast and long-context estimates follow the corresponding published rates. Cache writes for GPT-5.6 and GPT-6 use the published 1.25× input rate when a complete token breakdown is available. Saved old default Sol rates are replaced, custom prices remain, and historical estimates recalculate in the background.

- **Pelican tests and wakeups run independently of the API Service**: Pelican tests again send requests directly to the official upstream. Both features prepare the selected account without API Service cache or rejection state and use account-bound or global/system proxy settings instead of API Service network settings; service scheduling, cooldowns and sidecar availability no longer control them.

- **Prevent focus-induced scroll jumps in scrollable dialogs**: anchor the Codex provider-model sync switch and related import, sensitive-export and wakeup account-selection controls to their visible rows.

- **Fixed duplicate streamed replies through provider gateways**: a message now keeps the same ID from incremental output through completion, preventing clients from treating the final content as a second reply. Third-party ID format normalization is preserved.

- **Keep Codex OAuth and Grok provider accounts separate when they share an email**: OAuth authorization and token import no longer overwrite provider accounts. Reimporting credentials from a previously mixed record saves an independent OAuth account while retaining the old record and its references.

- **Fixed API-service Recover immediately marking the account unavailable again**: Recover now uses the same membership save path as unchecking then re-adding the account, so the local gateway reloads that account as a new member instead of only clearing scheduler flags on the still-loaded credential.

- **Fixed third-party supplier settings leaking into official Codex accounts after switching back**: writing a third-party or self-hosted supplier account into a Codex profile used to leave `model_provider` and its `[model_providers.<id>]` entry in `config.toml`, while the official-account projection keeps unknown provider ids as user-owned configuration. The client therefore kept sending requests to the previous supplier even though the account had already been switched back, and the UI kept showing that supplier. Profile writes now snapshot the previous `model`, `model_provider` and provider entry, and restore them when switching back to an official account; the record is per profile and independent of any supplier id. Successive provider switches cannot restore a removed managed provider, and later manual provider/model choices are preserved.
